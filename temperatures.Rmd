...
```{r Create STFDF object, echo=TRUE, message=FALSE}
...
lag8_spatial_var_model <- vgm(10, "Exp", 60000)
...
separable_fitted <- fit.StVariogram(sampl.var, separable, fit.method=11,method="L-BFGS-B", stAni=1, lower=pars.l,upper=pars.u)
attr(separable_fitted, "MSE")
# ------------------------------------------------------------------------------
# Product sum model
prodSumModel <- vgmST("productSum", space = vgm(10,"Exp",60000,0),time = vgm(psill=500,"Sph", range=5000, nugget=0), k=500)
prodSumModel_fitted <- fit.StVariogram(sampl.var, prodSumModel,method = "L-BFGS-B",lower=pars.l)
attr(prodSumModel_fitted, "MSE")
# ------------------------------------------------------------------------------
# Metric
metric <- vgmST("metric",joint=vgm(10,"Exp",60000,0),stAni=1)
metric_fitted <- fit.StVariogram(sampl.var, metric)
attr(metric_fitted, "MSE")
# ------------------------------------------------------------------------------
# Sum metric
sumMetric <- vgmST("sumMetric", space = vgm(10,"Exp",60000,0),time = vgm(psill=500,"Sph", range=5000, nugget=0), joint = vgm(25,"Exp",60000,0), stAni=500) 
sumMetric_fitted <- fit.StVariogram(sampl.var, sumMetric, method="L-BFGS-B",lower=pars.l,upper=pars.u,tunit="hours")
attr(sumMetric_fitted, "MSE")
# ------------------------------------------------------------------------------
# Simple sum metric
SimplesumMetric <- vgmST("simpleSumMetric",space = vgm(10,"Exp",60000,0),time = vgm(500,"Sph", 5000, 0), joint = vgm(25,"Exp",60000,0), nugget=1, stAni=500)
SimplesumMetric_fitted <- fit.StVariogram(sampl.var, SimplesumMetric,method = "L-BFGS-B",lower=pars.l)
attr(SimplesumMetric_fitted, "MSE")
# ------------------------------------------------------------------------------
# Plot those which give result
