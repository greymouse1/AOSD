...
stplot(st.winter,main="Winter",mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))
stplot(st.winter,main="Winter",ylab="Time",xlab="Temperature", mode="tp")
stplot(st.winter,main="Winter",ylab="Time",xlab="Temperature", mode="ts")

# Spring
st.spring <- STFDF(station_coords,season.time,data=data.frame(as.numeric(spring.averaged$AVERAGE)))
stplot(st.spring,main="Winter", mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))
stplot(st.spring,main="Winter",ylab="Time",xlab="Temperature", mode="tp")
stplot(st.spring,main="Winter",ylab="Time",xlab="Temperature", mode="ts")

# Summer
st.summer <- STFDF(station_coords,season.time,data=data.frame(as.numeric(summer.averaged$AVERAGE)))
stplot(st.summer,main="Winter", mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))
stplot(st.summer,main="Winter",ylab="Time",xlab="Temperature", mode="tp")
stplot(st.summer,main="Winter",ylab="Time",xlab="Temperature", mode="ts")

# Autumn
st.autumn <- STFDF(station_coords,season.time,data=data.frame(as.numeric(autumn.averaged$AVERAGE)))
stplot(st.autumn,main="Winter", mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))
stplot(st.autumn, main="Winter",ylab="Time",xlab="Temperature",mode="tp")
stplot(st.autumn, main="Winter",ylab="Time",xlab="Temperature",mode="ts")
```
Here I averaged readings for all seasons as well as for year(without taking seasons into account), so correlation will be calculated with years on x-axis and average temperature from all stations for a given season (or for full year).
```{r Correlation coefficients, echo=TRUE, message=FALSE}
# Whole year
year.mean <- aggregate(agg4$MEAN_TEMP, list(agg4$YEAR), mean)
