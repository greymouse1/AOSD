...
```{r Aggregate by month, echo=TRUE, message=FALSE}
...
stplot(st.winter, mode="ts")
stplot(st.winter, mode="tp")
stplot(st.winter, mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))

# Spring
st.spring <- STFDF(station_coords,season.time,data=data.frame(as.numeric(spring.averaged$AVERAGE)))
stplot(st.spring, mode="ts")
stplot(st.spring, mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))

# Summer
st.summer <- STFDF(station_coords,season.time,data=data.frame(as.numeric(summer.averaged$AVERAGE)))
stplot(st.summer, mode="ts")
stplot(st.summer, mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))

# Autumn
st.autumn <- STFDF(station_coords,season.time,data=data.frame(as.numeric(autumn.averaged$AVERAGE)))
stplot(st.autumn, mode="ts")
stplot(st.autumn, mode = "xt", cuts=11,scales = scales, xlab = NULL,col.regions=rev(brewer.pal(11, "RdBu")))
...
```{r, echo=TRUE, error=FALSE, warning=FALSE, fig.align="center"}
# Create spatial grid
