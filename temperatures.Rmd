...
To make things coherent, we will use only the same stations (I'm not sure if this is right, difference is in only one station anyway), so I will remove from observation year 2005 (it has only 13 stations), and I will remove one station for years which have data from 32 stations, so all years can have 31 station. We can see station which is missing for some years is station 14324.
Let's include only data we need:
```{r, echo=FALSE, message=FALSE}
aggregated_cleaned <- subset(aggregated, YEAR != "2005" )
aggregated_cleaned2 <- subset(aggregated_cleaned, STATION_ID != "14324")
```
Now we should have dataset without year 2005 and without station 14324. Let's check if that's correct:
```{r, echo=FALSE, message=FALSE}
checker(aggregated_cleaned2)
```
...
Create space-time object STFDF
```{r, echo=FALSE, message=FALSE}
sp <- cbind(LON = agg$LONGITUDE[1:32], LAT = agg$LATITUDE[1:32])
row.names(sp) <- paste(unique(agg$STATION))
sp <- SpatialPoints(sp)
time <- as.POSIXct(unique(agg$YEAR))
values <- agg$TEMP
